...
describe("SMT Token Testing", function () {
  ...
  it("Swap Token between ETH to SMT and verify the smart contract balance and owner balance", async () => {
    ...
    await SMTToken.mint({
      from: owner.address,
      value: ethers.utils.parseEther(ethTobeTransfered),
    });
    ...
    expect(parseInt(ethTobeTransfered) * parseInt(exchangeRate)).to.equal(
      parseInt(afterBal)
    );
  });
  ...
  it("Testing the receive function", async () => {
    ...
    var result =
      +ethers.utils.formatEther(contractBalBeforeTransfer) + parseInt(1);
    expect(+ethers.utils.formatEther(contractBalAfterTransfer)).to.equal(
      result
    );
  });
});
